...
export async function postFoodLogFromNutritionalLabelScanner(
  input: SaveScannedFoodLogRequest,
) {
  const payload = {
...
export async function postFoodLogFromSolidFoodScanner(
  input: SaveSolidFoodLogRequest,
) {
  const payload = {
    account_id: String(input.account_id ?? "").trim(),
    food_name: String(input.food_name ?? "").trim(),
    ...
    protein_g: toNumber(input.protein_g),
    fat_g: toNumber(input.fat_g),
    detected_items: Array.isArray(input.detected_items)
      ? input.detected_items
      : [],
  };
